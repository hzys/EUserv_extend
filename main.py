...
def check(sess_id, session):
    ...
    for key, val in d.items():
        if val:
            flag = False
            print("ServerID: 不知道 Renew Failed!" )
    if flag:
        ...
        for k, v in SERVERS.items():
            if v:
                if not renew(sessid, s, passwd_list[i], k):
                    print("ServerID: 不知道 Renew Error!" )
                else:
                    print("ServerID: 不知道 has been successfully renewed!" )
            else:
                print("ServerID: 不知道 does not need to be renewed" )
        time.sleep(15)
        check(sessid, s)
        time.sleep(5)
    print('*' * 30)
